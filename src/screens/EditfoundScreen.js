...
export default class EditfoundScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      selected2: undefined,
  ...
  async addPostfound() {
    ...
    data.append('action', 'editpostfound');
    data.append('foundid', this.props.navigation.getParam('found_id'));
    ...
    // console.warn(data)
    axios.post('http://192.168.0.111/lostandfound/api/addeditfound.php', data,
  ...
  componentDidMount() {
    ...
    axios.post('http://192.168.0.111/lostandfound/api/getfound.php', JSON.stringify({
      ...
      .then(response => {
        var coords = {
          latitude: response.data.found_latitude,
          longitude: response.data.found_longitude
        }

        this.setState({

          dataSourceFound: response.data,
          signupText1: response.data.found_topic,
          selected2: response.data.found_type,
          signupText3: response.data.found_detail,
          signupText4: response.data.found_location,
          gps: {
            coords: coords
          },
    ...
    // console.warn('dataSourceFound', this.state.dataSourceFound)
    this.getCategory();
    // this.getLocation();
        ...
        <Content>
          ...
          <CardItem cardBody style={styles.imageReadnews}>
            {this.state.ImageSource === null ?
              <Image source={{ uri: 'http://192.168.0.111/lostandfound/img_upload/found/' + this.state.dataSourceFound.found_img }} style={{ height: 200, width: null, flex: 1 }} /> :
